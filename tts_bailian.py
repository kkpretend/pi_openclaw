"""Bailian Qwen-TTS playback with pre-fetching for gapless sentence transitions."""
...
class TTSPlayer:
    ...
    def current_text(self) -> str:
        ...
        if not chars:
            return ""
        char_idx = min(int(progress * len(chars)), len(chars) - 1)
        end = char_idx + 1
        start = max(0, end - 8)
    ...
    def _fetch_wav(self, text: str) -> bytes | None:
        url = f"{config.BAILIAN_API_BASE_URL.rstrip('/')}/services/aigc/multimodal-generation/generation"
        headers = {
        ...
        payload: dict[str, object] = {
            ...
            "input": {
                "text": text,
                "voice": config.BAILIAN_TTS_VOICE,
                "language_type": config.BAILIAN_TTS_LANGUAGE,
            },
        }
        if config.BAILIAN_TTS_INSTRUCTIONS:
            payload["parameters"] = {
                "instructions": config.BAILIAN_TTS_INSTRUCTIONS,
        ...
        try:
            ...
            print(f"[tts] unexpected response: {resp.text[:200]}")
            return None
        ...
        try:
            ...
            print(f"[tts] audio download failed: {e}")
            return None

        wav_data = audio_resp.content
        content_type = audio_resp.headers.get("Content-Type", "")
        if not wav_data.startswith(b"RIFF"):
            print(f"[tts] expected WAV audio but got Content-Type={content_type or 'unknown'}")
            return None

        gain_db = config.BAILIAN_TTS_GAIN_DB
        if gain_db > 0:
            try:
                r = subprocess.run(
